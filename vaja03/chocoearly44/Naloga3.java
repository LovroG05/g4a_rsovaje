...
public class Naloga3 extends Application {
	private static final LinkedList<RTocka> kaca = new LinkedList<>();
	...
	public static void main(String[] args) {
		for(int i = 5; i < 12; i++) {
			kaca.add(new RTocka(i, 10, Color.RED));
		}
	...
	private static void move() {
		kaca.removeFirst();
		RTocka ls = kaca.getLast();
		kaca.addLast(new RTocka(ls.x() + glavaVektor.x(), ls.y() + glavaVektor.y(), Color.RED));
	...
	private static void redraw() {
		...
		kaca.forEach(rTocka -> canvas.getChildren().add(new Circle(rTocka.x() * 20, rTocka.y() * 20, 10, rTocka.barva())));
	}
